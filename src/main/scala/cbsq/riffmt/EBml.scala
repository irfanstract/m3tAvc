...
object EBml
{
   ...
   object Schema
   {
      ...
      case class OfFrame[
         +Len               <: S[cHasVariableLength.type | cbsq.FileSize], 
         +Children          <: false | true ,
         // +Children          <: OfMulti[?, ?, ?] | false ,
      ](  
         length: Len , 
         childrenSet1: Children ,
      ) 
      extends OfFixedLengthStrv
      {
      ...
      def ofFrame() = {
         OfFrame(length = cHasVariableLength, childrenSet1 = true)
      }
      ...
      @annotation.experimental
      def ofAnyChildren[ChildSchema <: Schema](c: Set[? <: ChildSchema]): Schema & OfFixedLengthStrv = {
         ofAnyChildren1(c = c)
      }
      @annotation.experimental
      def ofAnyChildren1[ChildSchema <: Schema](c: Set[? <: ChildSchema]) = {
         OfMulti(length = cHasVariableLength, childrenSet1 = c )
      }
