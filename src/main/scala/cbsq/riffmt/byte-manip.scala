...
object byteManipImplicits {
   ...
   def newMarkResetTurn(r: SicingInputStreamImpl, s: Int) = {
      r.enmark()
   }
   // /**
   //  * 
   ...
   // TODO
   type SicingInputStream
      >: SicingInputStreamImpl
      <: SicingInputStreamImpl

   extension (inp: java.io.InputStream) {
      def asSicingStream() = {
         inp match {
            case inp : SicingInputStreamImpl =>
               inp
            case _ =>
   ...
   class SicingInputStreamImpl(private val inp: java.io.InputStream) extends java.io.InputStream
   {

      private[SicingInputStreamImpl]
      var loadedSize = {
