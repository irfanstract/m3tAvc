...
{ EBml: (
   ...
   object CodeUnitScheme
   {
      ...
      def alwaysFailingInstance(h: () => String) = {
         new CodeUnitScheme
         {
            ...
            def readAndParse(r: RnpSource)(using CodeSchemeOps.TraversalDiagnostique): Instance = {
               throw summon[CodeSchemeOps.TraversalDiagnostique].newFMatException(msg = h() )
            }
         ...
         def readAndParse(r: RnpSource)(using CodeSchemeOps.TraversalDiagnostique) = {
            encodedLength match {
               case encodedLength : cbsq.FileSize =>
                  val b = cbsq.ByteBlob.copyOf(r readNBytes(encodedLength.bytes.toInt ) )
                  b encodedAs(enc = enc )
                  
               case _ =>
                  throw summon[CodeSchemeOps.TraversalDiagnostique].newFMatException(msg = "'encodedLength' not specified")
            }
         }
                  ...
                  val children = {
                     ...
                     val cp = ({
                        ...
                        LazyList() lazyAppendedAll {
                           ((_ : CodeSchemeOps.TraversalDiagnostique) ?=> {
                              val scheme = (

                                 classPayloadsTable
                                 .applyOrElse(className : BigInt, className => {
                                    throw java.util.NoSuchElementException((
                                       s"no scheme for cls ${className.toString(0x10) } "
                                    ))
                                 } )
      ...
      with (CodeUnitScheme.XLengthOverrideable)
      {
                        ...
                        if (noMore) {
                           ...
                           try 
                              ...
                              case z : java.io.EOFException =>
                                 ...
                                 throw (
                                    summon[CodeSchemeOps.TraversalDiagnostique]
                                    .newFMatException(msg = s"encountering EOF during (rather than right-before) parsing of a child. (collected: ${childrenLl })" , r = r )
                                 )
         ...
         extension (r: RnpSource) {
            def isAtEofRelevantly: Boolean = {
               // TODO
               isAtEof(r = r, lookaheadLimit = 20 * 1024 * 1024, tipSize = 0x2.B )
               // ??
            }
         }
   ...
   object schemeAlternativeImpl {
      ...
      def demarsh[
         ...
         s: ([I] =>> (Set[? <: I] | Seq[I] ) )[ChildScheme ] ,
      )(using CodeSchemeOps.TraversalDiagnostique)(r: SicingInputStream): Instance1 = {
            type Instance = Instance1
            /**
             * 
            ...
            util.control.NonLocalReturns.returning(resolve ?=> {
               ...
               throw summon[CodeSchemeOps.TraversalDiagnostique].newFMatException(msg = s"none match. ${s }")
            })
      }
