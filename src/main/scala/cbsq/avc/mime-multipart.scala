      ...
      def asMimeMultipartDecoded1(
          ...
          */
         {
            ...
            Iterator.continually({
                ...
                */
               srcByteReader.readByte() : Int 
            })
            .take(headerSizeLimitInBytes + 0x10 )
            .concat({
               throw (
                  new java.io.IOException(s"maximum header size (${headerSizeLimitInBytes}B) exceeded")
               )
            })
         ...
         if (payload.length < contentLength ) {
             ...
             * assertion error
             * 
             * we wanted to make it plain `IOException` rather than `EOFException`, but
             * this condition is normal in EOF(s)
             * 
             */
            throw new java.io.IOException({
               s"assertion error: (payload.length = ${payload.length} ) < (contentLength = ${contentLength }) "
            })
         }
          ...
          */
         {
               ...
               val s1 = {
                  ({
                     if {
                        ...
                        val bytes = src0.readNBytes(limit).nn
                        new java.io.ByteArrayInputStream(bytes )
                     }
                  } : (java.io.BufferedInputStream | java.io.ByteArrayInputStream ) )
                  .asXBufferedReader()
                  .linesSc()
                  .tapEachAndCheckTotalLengthWithinLimit(limit = limit )
                  .to(LazyList)
               }
            ...
            if limit < delimStr1.length() then {
               throw  new java.io.IOException(s"$limit vs ${delimStr1.length() } ")
            }
...
object mimeMultipartImplUtil
{
   ...
   extension [CC[E] <: collection.IterableOnceOps[E, CC, CC[E] ] ](src : CC[String]) {

      def tapEachAndCheckTotalLengthWithinLimit(limit: Long) = {
         ...
         src
               .tapEach({
                  ...
                  (s: String) => {
                     ...
                     if (limit < c.get() ) {
                        throw  new java.io.IOException(s"$limit exceeded ")
                     }
                  }
               })
