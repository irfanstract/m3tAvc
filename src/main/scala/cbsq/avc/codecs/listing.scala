               ...
               Some({
                  identity[XTraversiveDecoder]({
                     ...
                     case (src : java.io.InputStream, muxProperties) =>
                        new AnyRef with java.io.Closeable with WithStreams[InDemuxStreamMap ]
                        {
                           ...
                           val streams = {
                              // TODO
                              InDemuxStreamMap.enlist({
                                 Seq[(Int, MediaKind, InMuxStream)](
                                    (1, MediaKind.Video, frameIterator.asInstanceOf[InMuxStream] ) ,
                                 )
                              })
                           }
