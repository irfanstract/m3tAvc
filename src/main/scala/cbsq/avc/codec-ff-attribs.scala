         ...
         extension [A <: Args](a: A) {
            ...
            def forUrl(s: mcdcSupertraits.JUrlOrPath)(using ArgsDerivCompletibility[A, a.type, ardtForUrl.type] ): Args = {
               ...
               val s2 = {
                  ...
                  .toString()
                  match { 
                     case FfRawStreamCodecTemplateOps.ReadingOrWritingUrl(whichDir, s) =>
                        FfRawStreamCodecTemplateOps.ReadingOrWritingUrl(whichDir, s) 
                     case s =>
                        FfRawStreamCodecTemplateOps.ReadingOrWritingUrl("read", s) 
                  }
...
object FfRawStreamCodecTemplateOps
{

   object ReadingOrWritingUrl {

      val regex = "\\A(read|write):([\\S\\s]*)\\z".r

      export regex.unapplySeq

      def apply(k: (
         // "read" | "write"
         String
      ), v: String ) = {
         val whichDir @ ("read" | "write") = {
            k : @unchecked
         }
         s"$whichDir:$v"
      }
 ...
 * 
 */
trait FfInMuxStreamCodecTemplateOps extends
AnyRef
with FfCodecTemplateOps
with MediaCodecPropertiesMixin
{
 ...
 * 
 */
trait FfUnpacketisedInMuxStreamCodecTemplateOps extends
AnyRef
with FfInMuxStreamCodecTemplateOps
with MediaCodecPropertiesMixin
with FfRawStreamCodecTemplateOps
{
