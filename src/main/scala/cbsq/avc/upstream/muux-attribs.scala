...
object codecExtraOps
{
   ...
   extension (fmt1 : cbsq.avc.MediaDeviceOverview ) {
   ...
   extension (fmt1 : cbsq.avc.MediaDeviceOverview & cbsq.avc.MediaCodecProperties ) {
...
def getAllRegisteredDemuxers()(using CodecListDependentCtx) : collection.Iterable[cbsq.avc.MediaCodecOverview & cbsq.avc.FfCodecTemplateOps ] = {
   notImplemented
}

def getAllRegisteredDevices()(using CodecListDependentCtx) : collection.Iterable[cbsq.avc.MediaCodecOverview & cbsq.avc.FfCodecTemplateOps ] = {
   notImplemented
}
