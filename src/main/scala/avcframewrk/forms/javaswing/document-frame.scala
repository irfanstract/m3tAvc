...
extends java.io.Closeable
{
   ...
   val undoAac = xSwing.renderAbstractAction(label = "Undo", callback = { case _ => model.undo() })
   val redoAac = xSwing.renderAbstractAction(label = "Redo", callback = { case _ => model.redo() })

   undoStackChgItr
   .foreach(s => {
      import language.unsafeNulls
      extension (eaac : undoAac.type | redoAac.type ) {
         def updateToStack(title: String, stack: s.undoStates.type | s.redoStates.type ): Unit = {
            eaac setEnabled(stack.nonEmpty )
            eaac putValue(javax.swing.Action.NAME, s"$title (${stack.length })" )
         }
      }
      undoAac updateToStack(title = "Undo", stack = s.undoStates )
      redoAac updateToStack(title = "Redo", stack = s.redoStates )
   })
