...
lazy val avcBrowserWindowFormsProject
=
  (crossProject(suggestedTargetPlatforms : _* ).withSuggestedSettings() in (packagesParentDir / "avfwforms-on-browserwindow" ) )
  .asLeafProjectWithNecessarySettings()
  .dependsOn(avcGenericFormsProject )
  // .dependsOn(avFwHeadlessUtilityLibProject )
  .dependsOn(avcAlgebraLibProject )
...
=
  ...
  // .dependsOn(avcEvLibProject )
  .dependsOn(avcGenericFormsProject )
  .dependsOn(avcBrowserWindowFormsProject )
