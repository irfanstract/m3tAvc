         ...
         = {
            ...
            .map(s => ({
               ...
               new AnyRef with Selectable {
                  ...
                  def fullPropsType
                  = {
                     util.Try({ fullPropsTypeRepr.asType })
                     .recover({ case (z: MatchError ) => throw new AssertionError(z) } )
                     .get
                  }
                  ...
                  def selectPropNamesAsPropFactoryExpr
                     ...
                     (f: collection.immutable.Iterable[String] => Seq[String ] )
                  = {
                     val mt = selectPropNamesAsPropSList(f)
                     ;

                     selectPropNamesAsPropSList(f)
                     .pipeLooseSelf(sList => {
                        MethodType(sList.toList.map(_.name ) )(_ => {
                           sList.toList
                           .map(s => {
                              s.tree.asInstanceOf[ValDef].tpt.tpe.asType
                              match { case '[t] => TypeRepr.of[t] }
                           } )
                        } , _ => fullPropsTypeRepr )
                        .pipeLooseSelf(mt => {
                           ;
                           ...
                           Typed(Lambda(Symbol.spliceOwner, mt, (_, paramRefs) => {
                              ...
                              ('{ js.Dictionary(${
                                 Expr.ofSeq({
                                    ...
                                    .map({
                                       case (s, sI) =>
                                          val sName = s.name
                                          '{ ${Expr(sName ) } -> ${paramRefs.apply(sI ).asExpr } }
                                    })
                                    .map(<:<.refl )
                                 })
                              } : _* ) })
                              ...
                              .pipeLooseSelf({ case tRepr => TypeTree.of(using tRepr.asType ) })
                           } )
                           .asExpr
                        })
                     })
