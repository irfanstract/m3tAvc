...
extends
AnyRef
with EbFreezeableElements
{
   this : (
      AnyRef
      & EbAndReconcilability
      & EbFreezeableElements
   ) =>
   //
            ...
            // TODO
            {
               ...
               (0 : Int) match {
                  ...
                  case _ =>
                     import laminar.api.L
                     e.devLaminarWrapperEH
                     .amend(L.disabled <-- stateCheckedOption.map({ case v : java.lang.Number => 0.33 < v.doubleValue() }).toLaminarObservable() )
               }
