...
with SEnablednessDefs.OfHalfUnitScales
{
   ...
   given [State, Title]
   : SpawnabilityAndReconciliabilityNoArg[Edsb[State, Title], ?, Unit]
   = {
      ;
      class Lx()
      extends AnyRef
      with java.io.Closeable
      {
         ...
         val titleValueUpdatePipe = new ReroutiblePipe1[Title]
         val enablednessUpdatePipe = new ReroutiblePipe1[Boolean]

         def implModelSwitch(newModel: Edsb[State, Title] )
         : Unit
         = {
            ;
      ...
      new AnyRef with SpawnabilityAndReconciliabilityNoArg[Edsb[State, Title], Lx, Unit]
      {
         ;
         
         extension (this1: Edsb[State, Title] )
            def spawn()
            = {
               val e = new Lx
               e.model_=(this1)
         ...
         extension (this1: Lx )
            def model_=(newModel: Edsb[State, Title] )
