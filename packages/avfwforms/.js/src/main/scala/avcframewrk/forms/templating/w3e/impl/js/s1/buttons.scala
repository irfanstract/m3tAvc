   ...
   with w3e.pre.Buttons
{
   ...
   : SpawnabilityAndReconciliabilityNoArg[Edsb[?, ?] , SpawnedButton, Unit ]
   = {
      ;
         ...
         new Reconciliability[SpawnedButton, ButtonContentModel, Unit]
         {
            extension (this1: SpawnedButton)
               ...
               = {
                  import laminar.api.L
                  /**
                   * there's currently no way to undo the binding of `Observable`s done via `.amend` ;
                   * as a workaround,
                   * we replace with new container altogether
                   * 
                   */
                  this1
                  .amend(L.child <-- ??? )
