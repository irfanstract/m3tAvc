   ...
   with w3e.pre.Articles
{
   ...
   private[avcframewrk]
   def laminarAnchorSpawnabilityAndReconciliability
   = {
      ;
      ...
      type XModel
      >: (Option[(java.net.URI, Boolean)], Boolean )
      <: (Option[(java.net.URI, Boolean)], Boolean )
      ...
      with aBackreferencings.XEAndStateBagCm[XModel, Unit]
      {
         ...
         val forEnabledness = (L.disabled).startAttribNow(initialValue = false ).contramap((v: Boolean) => L.Val(v == false ) )

         override
         def model_=(newMdl: XModel): Unit
         = {
            forEnabledness.onNext((newMdl._1.nonEmpty ) )
            forHref.onNext(L.Val(newMdl._1.map(_._1).map(_.toASCIIString().nn ).getOrElse("") ) )
         }
