                     ...
                     new SrcLensAndDestAttrPairRawSelInvar[XModel]
                     {
                        ...
                        def apply
                           ...
                           = {
                              ...
                              = {
                                 L.Var[Option[V] ](None )
                                 match { case e => (e.writer.contramapSome, e.signal.map(o => o.getOrElse(defaultValue ) ) ) }
                              }
                              ;
                              f.dest match {
                              //
                              case com.raquo.laminar.receivers.ChildReceiver =>
                                 ...
                                 .amend("ch", L.child <-- (
                                    (
                                       aPiper._2
                                       // L.EventStream.periodic(15 * 1000 ).flatMap(_ => aPiper._2 ) /* a work-around */
                                       // L.EventStream.periodic(15 * 1000 ).map(_ => defaultValue )
                                 ...
                                 ({
                                    ...
                                    // aPiper._1
                                    L.Observer.fromTry(e => aPiper._1.onTry(e) )
                                    // .contramap((src: Any ) => ??? )
                                    // .contramapTry({
                                    //    ([E] => () => (e: util.Try[E] ) => e.recover({ case z => {
                                    //       org.scalajs.dom.console.warn(z)
                                    //       throw z
                                    //    } }) )
                                    //    .apply()
                                    // })
