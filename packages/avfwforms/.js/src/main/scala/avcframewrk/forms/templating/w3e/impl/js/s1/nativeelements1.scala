...
with ELaminarQckCoreABackreferencings
{
   ...
   def apply
      ...
      (srcToSetterPairs: (SrcLensAndDestAttrPair[XModel, ? ] )* )
      (
   ...
   = {
      ...
      with aBackreferencings.XEAndStateBagCm[XModel, Unit]
      {
         ...
         = {
            for ((f : SrcLensAndDestAttrPair[md, v] ) <- srcToSetterPairs )
            yield {
               ;
               val aPiper
...
val _ = {
   ...
   locally {
      ...
      given_SpawnabilityAndReconciliability_CaseClassGeneralised1(
         //
         ((_: Tuple3[?, String, ?] )._2, "(unknown)", laminar.api.L.idAttr )
         ,
         ((_: Tuple3[Boolean, String, BigInt] )._1, false, laminar.api.L.checked )
         ,
   ...
   locally {
      ...
      given_SpawnabilityAndReconciliability_CaseClassGeneralised1(
         //
         ((_: Tuple3[?, java.net.URI, ?] )._2.toASCIIString().nn , "(unknown)", laminar.api.L.href )
         ,
         ((_: Tuple3[Boolean, ?, BigInt] )._1, false, laminar.api.L.hidden )
         ,
