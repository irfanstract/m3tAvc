      ...
      extension (dataInBase64: String )
         ...
         = {
            ...
            .pipeLooseSelf(e => {
               ...
               //    .replace("+", "-")
               //    .replace("/", "_")
               //    .pipeLooseSelf(dom.decodeURIComponent(_) )
               // })
               dom.atob(e)
            } )
            // .tapLooseSelf(d => {
            ...
            // })
            .pipeLooseSelf((originalStr: String) => {
               ;
               /** `Char`s become JS `number`s */
               ({
                  originalStr
                  .toIndexedSeq
                  // /* spare alternative code in case the above were misbehaving */
                  // .pipeLooseSelf(str => IArray.tabulate[Char] (str.length() )(str.charAt(_) ).toIndexedSeq )
               })
               .tapLooseSelf(chrs => {
                  assert(new String(chrs.toArray[Char]) == originalStr, "unexpected String malformation during 'toIndexedSeq'" )
               } )
            } )
            .pipeLooseSelf(a => {
               /* broken */
               ...
               //    ;
               //    js.Array(a*)
               //    .pipeLooseSelf((d: js.Array[Char] ) => {
               //       new dom.Uint8Array(d.asInstanceOf[js.Array[Double] ] )
               //    })
               // })
               ({
                  ;
                  js.eval({
                     s"new Uint8Array(${
                        IArray.tabulate[Int] (a.length )(a.apply(_) )
                        .mkString("[", ",", "]")
                     }) "
                  })
                  .asInstanceOf[dom.Uint8Array ]
               })
            } : dom.Uint8Array )
            .tapLooseSelf(d => {
               dom.console.debug("d:", d )
