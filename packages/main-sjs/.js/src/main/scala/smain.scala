...
import avcframewrk.forms.templating.reactjs.IntrinsicElementSelectable.PropStModifier
import typings.react.mod.{Attributes as StAttributes }
import typings.react.mod.{HTMLAttributes as StHtmlAttributes }
object reactjs1 {
...
= {
   ...
   locally {
      ...
      ({
         ...
         // KS.SlClsAndApplyArgs.applyDynamicNamed(using defaultJsxImpl ) (<)("???")( )
         // KS.SlClsAndApplyArgs.applyDynamicNamed(<)("???")( )
         r
         .render({
         ;
         sMainApp.render()
...
object sMainApp {
   ...
   @scalajs.js.annotation.JSExportTopLevel(name = "render", moduleID = "app")
   def render()
   = {
      ;
      ...
      ({
         ...
         @deprecated
         lazy val unsafeEh = {
            import ReactWithinRender.Efb
            import ReactWithinRender.ReSubscriptiveHook._When
            import ReactWithinRender.ReSubscriptiveHook.uvImplicits.{*, given }
            ReactWithinRender.Efb.doneByFnc1((_: Any) => (locally[java.io.Closeable] (() => {} ) ) , Nil :+ "Tuple" )
            .useReSubscriptiveEffectHook(onEvtDispatchMdWhen = _When.Deferredly() )
         }
         val Button1
         ...
         <.div((
            Nil
            :+ <+.apply("material-ui-button", `type` = "button", dateTimeStr )
         ))
         <.div((
            Nil
            ...
            :+ (<<.button).decribeInstanceWithStModifyingFnc[StHtmlAttributes[?] ] ((_ ).setChildren(dateTimeStr) )
         ))
         })
      })
   }
