   ...
   (cps.async[JSFuture ] {
      ;
      {
         ...
         if false then {
            ...
            = avcframewrk.forms.domNavigatorObj.storageObj.watcher.ext.toStartSomeNodeFrp.FsWatch.CtxtualSetterStyle.defaultInstance[Nothing]

            avcframewrk.forms.domNavigatorObj.storageObj.watcher.ext.toStartSomeNodeFrp.FsWatch.startDirFrp({
               ;
...

         {
            ...
            while true do {
               ...
               cps.await({
                  (([R] => (f: (tInSeconds: Double) => R ) => f ) (tInSeconds => (
                     //
                     new js.Promise[Unit]((r, _) => {
                        js.timers.setTimeout(tInSeconds * 1000 ) { r(() ) }
                     })
                  ) ) )
                  .apply(32 )
               })

               cps.await(storageProxy.tryWriteOver("otherIndex.txt")(s"updated (${js.Date() }) other index file") )
