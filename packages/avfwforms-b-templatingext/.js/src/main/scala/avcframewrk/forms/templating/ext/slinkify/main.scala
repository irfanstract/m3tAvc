...
object publicly
{
  ...
  = ${ eclMacroImpl(renderContents1E = 'renderContents1, shallDebug = false ) }
  ...
  = ${ eclMacroImpl(renderContents1E = 'renderContents1, shallDebug = true ) }

  private
  def eclMacroImpl
    [Props : Type, E <: slinky.core.facade.ReactElement : Type ]
    (using Quotes)
    (renderContents1E: Expr[(props: Props) => E ], shallDebug: Boolean )
  = {
    ;
    ...
    = {
      ...
      else templating.ext.reactjsify.eclFrontEnd.EopDebugConfig.productionModeInstance
    }
