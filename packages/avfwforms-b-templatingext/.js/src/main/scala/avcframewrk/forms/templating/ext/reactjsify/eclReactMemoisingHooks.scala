...
object eclReactCachingHooksImpl {
   ...
   = {
      ...
      literateUcvHookFunction
         [R]
         ()
   } : Expr[ByRQueryAndDependenciesArrayThenDoAndReturnR[R]]
   ...
   /** 
    * code properly
    * the UCV hook call site thunk
    * 
    */
   def literateUcvHookFunction
      //
      [R : Type ]
      (using Quotes )
      ( )
   = {
      ...
      lazy val finalExpr = {
         ...
         generalisedSlotRefFlexed
            //
            (('{
               ;

               literatedUcvHookGsrLiftedCe
                  [R]
                  ( )
            }) , assumedPos = Position.ofMacroExpansion )
      }
   ...
   // private
   def literatedUcvHookGsrLiftedCe
      //
      [R ]
      ( )
      //
   = {
      ...
      (OCA: onRtAllocator.SHEWM._Carrier._Any ) ?=>
         ...
         locally[(
            (initialValue: DummyImplicit ?=> R, updatedDependenciesArray: DependenciesArray ) =>
               R
         )] :
            (initialValue, updatedDependenciesArray ) =>
               runUcvInnerHookCallFor
