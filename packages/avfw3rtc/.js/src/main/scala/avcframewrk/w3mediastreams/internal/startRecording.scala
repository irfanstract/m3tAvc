...
object eisr {
   ...
   object ByConstructedMediaStreamRcCtx {
      ...
      with java.io.Closeable
      {
         ...
         final val triStateAnim *: _
         = {
            ;

            val sHold
            = airstream.state.Var[Option[SrTriState] ](None )

            def trySetTo
               (newS: SrTriState)
            = {
               sHold.update({
                  case s0 @ Some(SrTriState.Terminated() ) =>
                     s0
                  case _ =>
                     Some(newS )
               })
            ...
            addEventListener_resume(typings.std.stdStrings.resume, { case _ => {
               trySetTo(SrTriState.Running() )
            } } )

            addEventListener_pause(typings.std.stdStrings.pause, { case _ => {
               trySetTo(SrTriState.Suspended() )
            } } )

            addEventListener_stop(typings.std.stdStrings.stop, { case _ => {
               trySetTo(SrTriState.Terminated() )
            } } )

            ({
               sHold.signal
               .map[DummyImplicit ?=> SrTriState ] (s => s.getOrElse(SrTriState.Suspended() ) )
            }, () )
